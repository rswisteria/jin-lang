...
def test_build_writes_the_bundle(tmp_path: Path) -> None:
    out = tmp_path / "dist"
    ...
    assert "Phase 4" in result.stderr  # プレイヤーはまだ無い
...
def test_build_single_is_refused_until_the_player_exists(tmp_path: Path) -> None:
    result = invoke("build", FIB, "--out", tmp_path / "dist", "--single")
    assert result.exit_code == 1 and "Phase 4" in result.stderr
