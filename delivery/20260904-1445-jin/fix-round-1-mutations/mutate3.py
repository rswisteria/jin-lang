...
MUTATIONS = [
    ...
    (
        ...
        [
            "packages/jin-cli/tests/test_cli.py",
            "-k",
            "read_only_directory or neither_file_nor_directory",
        ],
    ),
    (
        "N2b",
    ...
    (
        ...
        "### 3.6 （削除）",
        [
            "tests/spec/test_spec_consistency.py",
            "-k",
            "string_constraints or control_character_rule",
        ],
    ),
    (
        "N-2b",
...
results.append(
    (
        "N-02",
        "赤 (期待どおり)" if proc.returncode != 0 and not skipped else "!!! 緑のまま !!!",
        tail[-1] if tail else "",
    )
)
