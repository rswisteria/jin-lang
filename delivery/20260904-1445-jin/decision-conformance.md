...
**残存**: (1) `.jin` の隣にあるトレースを自動では拾わない（毎回ユーザーが選ぶ）。
(2) サーバがトレースを保持する経路が無いので、**スクラブのたびに全行を ws で送り直す**
（v1 の想定では問題にならないが、モデル出力の大きい長いセッションでは 1 回が MB 単位になる）。
(3) `<input type="file">` は同じファイルの再選択で `change` を出さないため、
`onChange` で `value` を空に戻している（Playwright の `setInputFiles` は常に発火するので自動検査では守られない）。
...
## 2.28 Issue #9（fix-later 7 件）で確定したこと（2026-09-07・toyota）

`DP-REVIEW-JIN-001`〜`007`。**判断の前に現状を実測**し、指摘の前提が今も成り立つかを確かめてから
選択肢を出した（2 件は前提が失効していた）。

| DP | 判断 | 実測に基づく決め手 |
|---|---|---|
| 001 | **対応する** | `rglob` はディレクトリ symlink を辿らないが**ファイル symlink は拾って読む**（Python 3.14.6）。範囲外ファイルの存在・パース可否・JSON のキー名が診断に載ることを再現した。`fmt` の書き込み側は 3 段で閉じていたが、**読み取り側は `check` / `fmt` とも素通り**だった |
| 002 | **対応しない** | ruff 0.16.6 の**既定**が 413 ルール / 38 prefix。求められていた I / B / UP は `select` 無しで既に走っている（本ランで I001 / ISC004 が検出された）。空ディレクトリでも同じ 413 件なのでローカル設定の混入ではない。CI は `UV_LOCKED=1 uv sync` で lock の 0.16.6 に固定 |
| 003 | **対応する**（Phase 4 で確定・実装済み） | `plugin` job（Phase 4）+ `editor` job（Phase 5）。2026-09-07 に**置換記録**で失効注記を訂正（§2.28.1） |
| 004 | **対応しない** | GitHub 公式と広く使われている action のみ。SHA ピンは dependabot をセットで入れないと腐る。方針が変わればセットで入れる |
| 005 | **対応する** | 直書きは 3 ファイル（src 側のヒットは docstring・コメント）。Issue の指定どおり**解決する形**にした |
| 006 | **対応する** | 件数の下限は走査の破損は拾うが、**定数を下げる行為そのものは検出されない** |
| 007 | **対応済み** | Phase 3 の correctness review R2-2 で改名済み。Issue が求めていた対応がそのまま実施されていた |

### 2.28.1 `DP-REVIEW-JIN-003` の置換記録

Phase 4 で `decision_record[]` に**手で**足したため `undecided[]` に残り、
`docs/pending-decisions.md` の未決と決定済みの**両方に出ていた**。あわせて rationale に
「`/aid decide` による再生成は未実施」という**失効した注記**があった（PR #22 で解消済み）。
`record.py` の置換記録（`action: replaced`・`chosen` / `decided_at` 不変）で両方を直した。
Phase 3 の `DP-IMPL-JIN-P3-LOOP-STAR-ORDER-01` と同じ扱いだが、DP-REVIEW 系は ADR を切らないので
新規発番は起きていない。

### 2.28.2 001 の線引き — 走査と名指しは別

**走査側にだけ**フィルタを置いた。問題は「走査が指定された範囲を越えること」であって、
ユーザーが名指ししたファイルではない。`_collect` の非対称そのものを
`test_collect_filters_symlinks_only_when_walking` が両側から固定する
（名指しは落とさない / 走査は落とす）。

**残存**: 判定と読み取りの間には窓がある（TOCTOU）。`fmt` の書き込みは下位の
`O_NOFOLLOW` / `os.replace` が競合なしで拒むが、**読み取りにはその段が無い**。
ここで塞いでいるのは「走査が範囲外のファイルを列挙して読むこと」であり、
競合を突いた読み取りまでは防いでいない。

### 2.28.3 005 — 自分の走査に引っかからない書き方

直書きを禁じるテスト（`test_no_test_hardcodes_a_delivery_run_directory`）は `tests/` 全体を
