...
MUTATIONS: list[tuple[str, str, str, str, list[str]]] = [
    ...
    ("ESC-surrogate-passthrough", CODEGEN,
    ...
    ("ESC-header-raw-source-name", CODEGEN,
    ...
    ("FAIL-builtin-circle-collision", CODEGEN,
     "        if circle.name in builtins:\n",
     "        if False:\n",
     [T_CODEGEN, "-k", "builtin_name_collision or named_like_a_builtin"]),
    # R1 F-C-P2-002: ADK ツール名の重複
    ("FAIL-adk-tool-dup", CODEGEN,
     "        if adk_name in seen:\n",
     "        if False:\n",
     [T_CODEGEN, "-k", "same_callable_name_in_one_circle or adk_tool_name_duplicate or same_adk_name"]),
    # R1 F-C-P2-016: root に親
    ("FAIL-root-parent", CODEGEN,
    ...
    ("BUILD-follow-out-symlink", BUILD,
    ...
    ("RUN-swallow-systemexit-in-run_model", RUNTIME,
    ...
    ("RUN-cancelled-to-runerror", RUNTIME,
    ...
    ("RUN-ignore-unanswered-tool", RUNTIME,
    ...
    ("RUN-await-pause-as-failure", RUNTIME,
     "    return [name for call_id, name in pending.items() if call_id not in long_running]\n",
    ...
    ("RUN-cancelled-passthrough", RUNTIME,
     "            if task is not None and task.cancelling():\n                # shutdown",
     "            if True:\n                # shutdown",
    ...
    ("RUN-cwd-stays-after-import", RUNTIME,
     "            with contextlib.suppress(ValueError):\n                sys.path.remove(entry)\n",
     "            pass\n",
     [T_CLI, T_RUNTIME, T_CLI_CONTRACT, "-k",
    ...
    ("TRACE-transfer-drops-siblings", TRACE,
    ...
    ("TRACE-transfer-call-as-tool", TRACE,
     "        if name == TRANSFER_TOOL_NAME:\n            continue\n",
     "        pass\n",
     [T_TRACE, T_RUNTIME, "-k", "transfer_function_call_is_not_a_tool_row or delegate_transfer_end_to_end"]),
    # R1 F-C-P2-007 / F-C-P2-021: text / error を捨てる
    ("TRACE-drop-text-with-call", TRACE,
    ...
    ("CLI-cancelled-traceback", CLI,
     "    except asyncio.CancelledError as exc:\n        # 保険（F-S-P2-202）",
     "    except asyncio.CancelledError as exc:\n        raise\n        # 保険（F-S-P2-202）",
     [T_CLI, "-k", "stray_cancelled_error"]),
    ("RUN-swallow-systemexit-at-runtime", CLI,
     "    except SystemExit as exc:\n        # ツール関数の sys.exit() は",
     "    except SystemExit as exc:\n        raise\n        # ツール関数の sys.exit() は",
     [T_CLI, "-k", "tool_sys_exit_at_runtime"]),
    ("CLI-trace-follow-symlink", CLI,
...
def _run_pytest(copy: pathlib.Path, target: list[str]) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [sys.executable, "-m", "pytest", "-q", "-p", "no:randomly", "--no-header",
         "-p", "no:cacheprovider", "-o", "addopts=--import-mode=importlib", *target],
        cwd=copy, capture_output=True, text=True, env=_env(copy), check=False,
    )
...
def _summary(result: subprocess.CompletedProcess[str]) -> str:
    ...
    return lines[-1] if lines else result.stderr.strip().splitlines()[-1:] and result.stderr.strip().splitlines()[-1] or "(no summary)"
...
def main() -> int:
    ...
    try:
        ...
        where = subprocess.run(
            [sys.executable, "-c", "import jin_adk, jin_cli; print(jin_adk.__file__); print(jin_cli.__file__)"],
            cwd=copy, capture_output=True, text=True, env=_env(copy), check=False,
        ...
        if not all(line.startswith(str(copy)) for line in where):
            print("!! jin_adk / jin_cli が隔離コピーを指していない。中止"); return 2
        baseline = _run_pytest(copy, ["packages/jin-adk/tests", T_CLI, T_GUARD])
        if not _is_green(baseline):
            print("BASELINE NOT GREEN"); print(baseline.stdout[-2000:]); return 2
        ...
        for name, rel, before, after, target in mutations:
            ...
            if name == "BUILD-pkg-symlink-both":
                mutated = mutated.replace("        if stat.S_ISLNK(info.st_mode):\n", "        if False:\n", 1)
                mutated = mutated.replace(
                    "os.O_RDONLY | os.O_DIRECTORY | os.O_NOFOLLOW, dir_fd=out_fd",
                    "os.O_RDONLY | os.O_DIRECTORY, dir_fd=out_fd", 1)
            ...
            if name in EXPECT_GREEN:
                ...
                ok = _is_red(result)
                status = "RED (expected)" if ok else ("GREEN (!! not caught)" if result.returncode == 0 else f"NOT RED (!! exit {result.returncode})")
            caught += ok
            print(f"{name:32s} {status:34s} {_summary(result)}")
        subset = f" (subset of {len(MUTATIONS)}; MUTATE_ONLY={','.join(sorted(only))})" if only else ""
        print(f"{caught}/{len(mutations)} mutations caught{subset}" + (f" ({skipped} skipped)" if skipped else ""))
