...
group("parseTrace", () => {
	test("`jin run --trace` の実出力を読む", () => {
		...
		expect(rows.map((row) => seqOf(row))).toEqual([
	...
	test("文字列欄の読み取り", () => {
		const rows = events(
			'{"seq": 1, "pointer": null, "kind": "model", "name": "m", "agent": 3}\n',
		);
...
group("eventsFiredAt", () => {
	...
	test("`pointer: null` の行は残らない", () => {
		...
		expect(
			eventsFiredAt(withNull, "/circles/0").map((row) => seqOf(row)),
		).toEqual([2]);
	});
	...
	test("別の紋の行は残らない", () => {
		expect(
			eventsFiredAt(rows, "/circles/1/flow/exit").map((row) => seqOf(row)),
		).toEqual([5, 8, 11]);
		expect(
			eventsFiredAt(rows, "/circles/2/core").map((row) => seqOf(row)),
		).toEqual([1]);
	});
});

group("appendRow", () => {
	test("行を足すと upto が最大まで伸びる", () => {
