import { maxSeq, parseTrace, type TraceEvent } from "../trace/parse";
...
export async function loadTrace(file: File): Promise<LoadResult> {
  ...
  try {
    ...
    return { ok: false, message: `${file.name}: 読めません（${messageOf(error)}）` };
  }
  const parsed = parseTrace(text);
  if (!parsed.ok) {
    return { ok: false, message: `${file.name}:${parsed.line}: ${parsed.message}` };
  }
  if (parsed.events.length === 0) {
    // 空のトレースは**正当**である（`docs/spec/layout.md` §7.4「空トレースは点 0 個」）。
    return { ok: true, replay: { name: file.name, events: [], upto: 0 } };
  }
  return { ok: true, replay: { name: file.name, events: parsed.events, upto: maxSeq(parsed.events) } };
}
