...
import { maxSeq, pointerOf, seqOf, stringOf, type TraceEvent } from "../trace/parse";
...
export function DebugPanel(props: DebugPanelProps): React.JSX.Element {
  ...
  return (
    <section className="jin-debug" data-testid="jin-debug">
      ...
      {replay === null ? (
        <p className="jin-hint">トレースを読み込むと、発火した要素が図に重なります。</p>
      ) : (
        <>
          ...
          <label className="jin-field">
            <span>
              upto <output data-testid="jin-upto-value">{replay.upto}</output> / {upper}
            </span>
            <input
              type="range"
          ...
          <label className="jin-check">
            ...
            <span>
              ...
              {props.selectedPointer === null ? (
                ...
                <code data-testid="jin-trace-selected">{props.selectedPointer}</code>
              )}
            </span>
          </label>
            ...
            {shown.map((event) => (
              <li key={event.line}>
                ...
                >
                  ...
                  <span className="jin-trace-kind">{stringOf(event, "kind") ?? "?"}</span>
                  <span className="jin-trace-agent">{stringOf(event, "agent") ?? ""}</span>
                  <span className="jin-trace-name">{stringOf(event, "name") ?? ""}</span>
                </button>
              </li>
            ))}
          </ol>
          ...
          {detail === null ? (
            ...
            <dl className="jin-trace-detail" data-testid="jin-trace-detail">
              <dt>kind</dt>
              <dd data-testid="jin-detail-kind">{stringOf(detail, "kind") ?? ""}</dd>
              <dt>name</dt>
              <dd data-testid="jin-detail-name">{stringOf(detail, "name") ?? ""}</dd>
              <dt>agent</dt>
              <dd data-testid="jin-detail-agent">{stringOf(detail, "agent") ?? ""}</dd>
              <dt>pointer</dt>
              <dd data-testid="jin-detail-pointer">{pointerOf(detail) ?? "null"}</dd>
              <dt>input</dt>
              {/* **そのまま**出す。要約も切り詰めもしない（モデル入出力を見るための面である）。 */}
              <dd>
                <pre data-testid="jin-detail-input">{render(detail.row["input"])}</pre>
              </dd>
              <dt>output</dt>
              <dd>
