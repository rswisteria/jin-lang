...
| 41 | 再生の行数と描き直し(Phase 6 で確定) | 走らせている間の行は #38 のまま(4000 で古い行を落とす)。録画の再生は落とさず `MAX_REPLAY_ROWS`(60000)を超えたら載せずに断る。描き直しはプレイヤーの `jin.status`(`running: false`)で即座に、走っている間は 1 秒に 1 回 | 古い行を落とすと `enter` 行(init 値)が消え、以後 `set` されない state の値が黙って狂う。再生はヘッダの `ticks` で有界なので上限を別に置ける。paddle 600 tick は約 7,000 行。ws の 1 メッセージの上限は `WS_MAX_MESSAGE_BYTES` = 64 MiB（`jin_lsp.server`。`MAX_REPLAY_ROWS` 60000 行 × `set` 行が数百バイトになる `.jin`（tetris の `Play.board`）で数十 MB。`websockets` の既定 1 MiB は tetris の録画の再生を数千行で閉じた） |
...
| 57 | 自陣の手順への `cast` の直後の中断検査を「呼ぶ前の生存」との比較にする(Issue #87 / #89) | プレリュードに `LIVE(i)`(呼ぶ前に active で休止中でないか)を足し、`STOP(i, live)` を「呼ぶ前は active で今は active でない」= **この呼び出しで陣が止まった**(`finish` / `transfer` した)ときだけ真にする。生成部は自陣の手順への `cast` の前に `local w = LIVE(i)` を出して直後の `STOP(i, w)` に渡す(wasm-GC は i32 の局所。`wait` を含む手順ではフレームの欄に持ち上げ、tick を跨ぐ `cast` でも呼ぶ前の値が残る)。JIL は 6 → 7(release の生成部が初めて動いた版)。呼ぶ前から active でない陣 = 未 entered の summon の呼び先(model.md §3.2)・既に `done` の陣・`on exit` の中・`transfer` で休止中の陣では入れ子の `cast` の後も打ち切らない。jil.md §1 / §4 | 元の `STOP(i)`(`status ~= "active" or paused`)は呼び出しの後の状態しか見ず、未 entered の呼び先を summon すると最初の入れ子の `cast` の直後で `output: null` のまま戻った(othello で実測・#87)。プレリュードだけで `idle` を除く形(#87 の案 1・PR #90)は `done` の陣と `on exit` の中を扱えない。呼び出し元の陣の添字で見る形(#87 の案 3)は summon の連鎖で添字の受け渡しが要る。呼ぶ前の生存を取る形は生成部が 1 行増えるが、判定が「この呼び出しで止まったか」という本来の意味と一致し、両経路とも局所 1 つで済む |
---
