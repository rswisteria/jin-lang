...
**残存**: 1 メッセージの上限は `WS_MAX_MESSAGE_BYTES` = 64 MiB（`jin_lsp.server`。`websockets` の既定 1 MiB は
録画の再生で `jin/renderSvg` に載せる `MAX_REPLAY_ROWS` = 60000 行のトレース（`set` 行が数百バイトになる
`.jin` で数十 MB）を数千行で 1009 で閉じ、エディタが「表示できません」になった）。同じマシンの任意のページが
1 メッセージ 64 MiB まで送れるが、メモリだけの局所的な負荷で、上の 2 本はトークン無しには通らない。
