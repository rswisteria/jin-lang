...
`jil: 7` は JIL の契約の版。プレリュードと生成部の**組**で 1 つの版とし、片方だけ変えない(v2.1 で 1 → 2: 生成部に
`restore` / `prestore` / `pdump` / `JR[k]` が、`tick` の結果に `snapshot` / `resume` が加わった。runtime.md §1.3。
2 → 3: プレリュードに `H.storage` / `F.num` が、`tick` の結果に `storage` が加わった。abilities.md §8。3 → 4: プレリュードに `F.cmp` が加わった。expr.md §4.1。4 → 5: プレリュードに `H.input.text` が加わり、`JS` の制御文字をロケールに依存する `%c` ではなく範囲で書くようにした。abilities.md §3。5 → 6: プレリュードに `ASK`(v1 の陣への問い)と入力イベント `reply` の配達が、`tick` の結果に `asks` が、`snapshot` に `asked` が加わった。runtime.md §11。6 → 7: プレリュードに `LIVE` が加わり、`STOP` が「呼ぶ前の生存」を受けて**この呼び出しで active でなくなったときだけ**真になる形に変わった。生成部は自陣の手順への `cast` の前に `LIVE(i)` を局所に取る。release の生成部が初めて動いた版。Issue #89)。
...
R[2][3] = function(l_0)  -- step          (陣 Play は circles[1] なので Lua では 2。手順 step は rites[2] なので 3)
  ...
  local w_5 = LIVE(2)       -- 自陣の手順への cast: 呼ぶ前の生存を取る
  R[2][4]()                 -- (rite 行は呼ばれた側が積む)
  if STOP(2, w_5) then return end   -- この呼び出しで active でなくなった(finish / transfer した)ときだけ巻き戻す
end
```
...
- `FINISH(i)` は陣を `done` にし、その手順から `return` する。`finish` の後の同じ手順内のステップは走らない。呼び出し元の手順(自陣の手順を `cast` した側)は、`cast` の前に取った `LIVE(i)`(active で休止中でない)と `cast` の直後の `STOP(i, live)`(呼ぶ前は active で、今は `done` か休止中)で**この呼び出しで陣が active でなくなった**ことを見て自分も `return` する(生成部が対で出す)。呼ぶ前から active でない陣では止めない: 未 entered の summon の呼び先(model.md §3.2)・既に `done` の陣・`FINISH` が `done` にした後に走る `on exit` の手順では入れ子の `finish` / `transfer` は no-op(`FINISH` / `TRANSFER` は `status == "active"` のときだけ動く)なので巻き戻す理由が無く、`transfer` で**休止中**の陣(status は `active` で `paused`。他陣からの summon でしか手順が走らない)で入れ子の `finish` が起きたときは陣は `done` になるが呼び出し列は巻き戻さない(#87 で保留した休止中の扱いはこう決めた。Issue #87 / #89)
...
- `game.wasm` = **ランタイム部**(`jin_wasmgc/runtime.wat`。プレリュードに相当。`scripts/generate_runtime_wat.py` が
  `packages/jin-wasmgc/runtime/` の部品と文字列表から生成する生成物で、data 区画の番地は生成器が振る)+ **生成部**(`.jin` から)を 1 つの
  module に連結して assemble したもの。先頭のコメントは `;; generated by jin — do not edit` / `;; source: "paddle.jin"` /
  `;; jin: 2  jil: 7  target: wasm-gc`。**`jil` の版は Lua 経路と共有する**(契約の版であって Lua の版ではない。プレリュードと
...
- **完了条件のパリティ**: `tests/fixtures/v2-programs/`(14 本)と `examples-v2/`(3 本 + 後から足した tetris / othello)の全部について、同じ seed /
