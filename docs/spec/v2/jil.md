...
- 局所変数と「どこまで進んだか」(再開点)をフレームの `struct` に持ち上げ、手順の関数は毎 tick 再開点から `br_table` で
  続きに飛ぶ(probe A.2)。`wait` に到達しない手順はふつうの関数のまま
- `cast` で辿った先の手順が `wait` するときは、呼び出し側のフレームも積む(フレームの配列 = Lua のコルーチンの呼び出し列)。
