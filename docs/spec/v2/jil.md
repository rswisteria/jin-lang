...
- `FINISH(i)` は陣を `done` にし、その手順から `return` する。`finish` の後の同じ手順内のステップは走らない。呼び出し元の手順(自陣の手順を `cast` した側)は、`cast` の前に取った `LIVE(i)`(active で休止中でない)と `cast` の直後の `STOP(i, live)`(呼ぶ前は active で、今は `done` か休止中)で**この呼び出しで陣が active でなくなった**ことを見て自分も `return` する(生成部が対で出す)。呼ぶ前から active でない陣(未 entered の summon の呼び先・model.md §3.2、既に `done` の陣、`FINISH` が `done` にした後に走る `on exit` の手順)では入れ子の `finish` / `transfer` は no-op なので止めない(Issue #87 / #89)
