...
`jil: 4` は JIL の契約の版。プレリュードと生成部の**組**で 1 つの版とし、片方だけ変えない(v2.1 で 1 → 2: 生成部に
`restore` / `prestore` / `pdump` / `JR[k]` が、`tick` の結果に `snapshot` / `resume` が加わった。runtime.md §1.3。
2 → 3: プレリュードに `H.storage` / `F.num` が、`tick` の結果に `storage` が加わった。abilities.md §8。3 → 4: プレリュードに `F.cmp` が加わった。expr.md §4.1)。
