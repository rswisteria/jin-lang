...
**既知の逸脱**: Lua の `%.<n>e` の探索は、往復の区間が非対称な **2 の冪**(仮数が 2^52 で下の隣との間隔が半分になる値。
`2^-1022` 未満の非正規化数を除く)で `repr` より 1 桁長い文字列を選ぶ(例: `2^-24` は repr の `5.960464477539063e-08` に
対して `5.9604644775390625e-08`)。両経路(Lua / wasm-GC)で同じ文字列を出すことを `tests/fixtures/numbers.jsonl`
(`scripts/generate_number_fixture.py`。`str` が正)が固定し、`str != repr` の行がその 2 の冪である。書式の正は
**Lua 経路の `NUMSTR`** で、`repr` との一致は非整数 700 件(2 の冪を含まない)にだけ効く。
...
- **`--target wasm-gc`**(v2.1・jil.md §6.7)は同じ引数・同じ出力で `game.wasm` を wasmtime で走らせる(`jin_wasmgc.runtime.run_headless_wasm`)。トレース / frames / 標準出力は Lua 経路とバイト一致(`tests/contract/test_wasmgc_parity.py`)。`agent` の答えは `jin_wasm.runtime.answer_asks` を共有する
...
- **`--target wasm-gc`**(v2.1・jil.md §6.8)は `game.lua` の代わりに `game.wasm` を書き、`game.manifest.json` に `target: "wasm-gc"` と `wasm`(sha256)を載せる(`jil` は無い。Lua 経路の manifest に `target` は足さない = 無ければ `"lua"`)。プレイヤーは `index.html` / `player.js` だけを並べる(`wasmoon.wasm` は Wasmoon のためのもので、wasm-gc には要らない)。`--single` は `window.JIN_BUNDLE = { manifest, game: base64 }`(`game.wasm` の base64。asset を拒む規律は同じ)で、ページは何も fetch しない(`apps/player/e2e/wasmgc.spec.ts` が実測)
...
- **wasm-GC のホスト**(v2.1・jil.md §6.2 / §6.8): `game.manifest.json` の `target` が `"wasm-gc"` なら `game.wasm` を fetch して `WasmGcHost`(`apps/player/src/host.ts`)で走らせる。`JinHost`(Wasmoon)と同じ `boot(seed, manifest)` / `tick(t, inputs)` の口(`Host`)を持ち、`Player` / `InputReducer` / `Recorder` / `jinrec.ts` はどちらかを知らない。`WebAssembly.instantiate(bytes, {})`(import は空)で instantiate し、JSON を `TextEncoder` でバイト列にしてから `input(n)` で入力域をもらい、**その後に** `memory.buffer` を取って書く(`input` が `memory.grow` すると前の `ArrayBuffer` は detach される・probe A.5)。`tick` の `(ptr, len)` を `TextDecoder` で読んで `JSON.parse`。サンドボックスの手続き(消すグローバル / hook)は無く、命令数の上限は module 内のカウンタ(jil.md §6.6)。wasm の trap は生成系のバグなので `HostError` で止める(パリティの対象外)。**エディタの `jin.load` は JIL のまま**(LSP の生成は Lua 経路・設計書 §8)。パリティは `apps/player/e2e/wasmgc.spec.ts`(録画 → Lua と wasm-GC の両方の `jin run --input` と全行一致)
