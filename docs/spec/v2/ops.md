...
- **本文に追加**(v2.1): 選択中の **1 つ**の `loop` / `if` の本文(`…/steps` / `…/then`)の**末尾**へ `addStep`(種別はパレット)。「ステップを追加」は選択の直後、ドラッグの落とし先は既にあるステップなので、**空の本文には図に要素が無く、この操作でしか入れられない**。足したステップを選ぶ。`else` と範囲選択には効かない(`apps/editor/src/v2/actions.ts` の `addStepInside` / `insideListOf`)
- **手順の引数の表**(v2.1): `Rite.params` は schema の印 **`x-jin-inline`**(`jin_core.v2.model.INLINE_SCHEMA_MARK`。`Rite.params` にだけ付く)で、フォームに行の表(`rowList`。列は要素の schema `Param` から生成)として出る。行を足す既定値は空き番の名前 + `num`(`addState` と同じ最小の妥当な値)。**名前だけ**が変わった行は `rename`(式の中の参照が追随する)、追加 / 削除 / 型の変更は `setRiteSignature` の 1 件。他の配列(state / rites / steps / sigils …)に印は付けず、図の操作のまま
- **イベントを追加**(Issue #95): 選択中の**手順**を呼ぶ `on` を `setOn` で足す。`event` は schema の `OnHandler.event` の enum のうち、その陣でまだ使われていない先頭(通常は `tick`)。全部使われていれば送らず理由を出す。足した `on` を選ぶ(フォームで `event` / `rite` を変えられる)。参照先を捏造しない(手順は選択中のもの)。`apps/editor/src/v2/actions.ts` の `addOn`
