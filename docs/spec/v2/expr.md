...
- 比較(文法の規則 `cmp`)は**連鎖しない**(`a < b < c` は JIN201)。規則の名前であって、文字列の順序を返す純関数 `cmp(a, b)`(§4.1)とは別物
...
| `<` `<=` `>` `>=` | num | num | bool | str には使えない(JIN202)。文字列の順序は純関数 `cmp(a, b)`(§4.1・v2.1)で `cmp(a, b) < 0` のように書く |
...
| `cmp` | str, str | num | 文字列の順序(v2.1)。`a` が先なら `-1`、同じなら `0`、後なら `1`。**コードポイント順**(= UTF-8 のバイト順。ロケールの照合順でも UTF-16 のコード単位順でもない: `"B"` は `"a"` より先、`"ｱ"`(U+FF71)は `"😀"`(U+1F600)より先)。前方一致は短い方が先(`"ab"` は `"abc"` より先)。並べ替えや辞書順の表に使う。§1 の文法の規則 `cmp`(比較)とは別物 |
...
2. **括弧**: 優先順位と結合で**必要なときだけ**(利用者の冗長な括弧は消える。`(a and b) or c` → `a and b or c`、`((n))` → `n`)。左結合の二項演算子は、左の被演算子が同順位なら付けず、右なら付ける(`a - (b - c)` は残り、`(a - b) - c` は `a - b - c`)。比較(文法の規則 `cmp`)は連鎖しないので両側とも比較なら付ける(`(a < b) == c`)。`not` の被演算子が `and` / `or`、単項 `-` の被演算子が二項式なら付ける。`-(-x)` は読みやすさのため付ける(`--x` と書いても `-(-x)`)。後置(`.` / `[]` / `()`)の対象が一次式・後置でなければ付ける(`(-a).x` / `(a + b)[0]`)
