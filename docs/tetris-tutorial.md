...
3. **変えてみる**: ステップを選ぶと右側のプロパティパネルに式が出ます。書き換えて Enter で確定すると、その場で実行パネルに反映されます（走っている途中の状態は保ったまま）。ファイルに書き戻すのは上部の「保存」ボタンです。次に開くときのために保存しておきましょう
...
1. マスの色を変えてください（`#f2d541` は黄色）。エディタで `paint` をダブルクリック → 色のステップを選ぶ → 右のパネルで式を書き換えて Enter → 「保存」
...
このような 1 行 1 イベントのファイルを `--input` で渡します。上は「tick 2 で Space を押し、tick 3 で離す」。エディタの実行パネルで**録画**を押してから遊び、**停止**すると、同じ形式のファイルがダウンロードされます。
