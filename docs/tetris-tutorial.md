...
1. **開いて遊ぶ**: `uv run jin editor docs/samples/tetris/<番号>-<名前>.jin` でエディタが開きます。右の実行パネルでゲームが動きます（画面をクリックしてからキーを押してください）
2. **図を読む**: 魔法陣の中の手順（`rite`）の名前をダブルクリックすると、その手順の**ステップの図**が開きます。本文の JSON 抜粋と見比べてください
3. **変えてみる**: ステップを選ぶと右側のプロパティパネルに式が出ます。書き換えて Enter、保存（Ctrl+S）すると実行パネルにすぐ反映されます
...
ブラウザに魔法陣と、右側に黒い画面（実行パネル）が出れば準備完了です。
...
- **核**（`core`）に指定した手順は、陣が始まるときに **1 回だけ**呼ばれます。長く動かしたければ、その中で待ったり繰り返したりします
...
`wait` は「指定した tick 数だけ待つ」。300 tick = 10 秒待って、核の手順が終わります。核の手順が終わると陣は「done」になり、`on tick` も呼ばれなくなるので、画面はそこで止まります。10 秒で止まるのはそのためです。

### 練習問題

1. マスの色を変えてください（`#f2d541` は黄色）。エディタで `paint` をダブルクリック → 色のステップを選ぶ → 右のパネルで式を書き換えて Enter → Ctrl+S
...
← → でマスが左右に動き、↓ を押している間は速く落ちます。底に着くと上に戻って繰り返します。実行パネルを一度クリックしてからキーを押してください。
...
`Result` の核 `show` は 20 tick 待つだけです。積み上がった直後のクリックを誤ってボタンに当てないための間です。
...
`assert` は「いつでも真のはずの式」。毎 tick 検査され、偽になるとデバッグモードのエディタにバッジが出ます。プログラムを止めるためではなく、**「自分がそう信じていること」を書いておいて、壊れたときに気づくため**のものです。練習問題で盤面を 12 列にしたら、最初の検査が教えてくれます。
...
このような 1 行 1 イベントのファイルを `--input` で渡します。上は「tick 2 で Space を押し、tick 3 で離す」。エディタの実行パネルの**録画**ボタンで遊ぶと、同じ形式のファイルがダウンロードされます。
...
エディタの**デバッグモード**（上部の切り替え）では、実行パネルで**録画**して遊び、**停止**してから「この録画を再生」を押すと、その録画の全トレースが読み込まれます。下のスライダーを動かすと、
...
走らせている間のトレースは 4000 行までで古いものから消えます（テトリスは 1 tick に約 90 行出るので、40 tick ぶん）。長く追いたいときは、録画を再生し直してください（6 万行まで読めます）。
